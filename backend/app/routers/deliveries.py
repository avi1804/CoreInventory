...
from app.services import delivery_service, auth_service
...
def add_delivery(
    ...
    """Add a delivery - requires authentication"""
    try:
        delivery_service.create_delivery(db, delivery)
        return MessageResponse(message="Delivery Added")
...
def get_deliveries(
    ...
    """Get all deliveries - requires authentication"""
...
def update_delivery_status(
    ...
    """Update delivery status: draft -> waiting -> ready -> done - requires authentication"""
...
@router.post("/validate/{delivery_id}", response_model=MessageResponse)
def validate_delivery(
    delivery_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(auth_service.get_current_user)
):
    """Validate delivery and update stock - transitions from ready -> done - requires authentication"""
    delivery = delivery_service.validate_delivery(db, delivery_id)
    if not delivery:
        raise HTTPException(status_code=400, detail="Delivery not found or already validated")
    return MessageResponse(message="Delivery validated and stock updated")
