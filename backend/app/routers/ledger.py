...
def get_product_ledger(product_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def get_warehouse_ledger(warehouse_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
